						...
						<div class="agileinfo-grap">
							<div class="agileits-box">
								...
								<div class="agileits-box-body clearfix">
									...
									<form action="execute" method="post" enctype="multipart/form-data" style="max-width: 600px; margin: 0 auto;">

									    <div style="margin-bottom: 10px; display: flex;">
									    	<span style="margin-right: 10px; width: 40%; padding: 12px; align-content: center;">Tên sản phẩm : </span>
									        <input type="text" name="editname" value="${name}"  placeholder="Nhập tên sản phẩm" required="required"
									    ...
									    <div style="margin-bottom: 10px; display: flex;">
									        ...
									        <textarea rows="5" cols="50" name="editdecs" required="required" style="width: 100%; padding: 12px; border-radius: 12px; margin-bottom: 10px; box-sizing: border-box;">${decs}</textarea>
									    </div>
									
									    <div style="margin-bottom: 10px; display: flex;">
									    	<span style="margin-right: 10px; width: 40%; padding: 12px; align-content: center;">Giá sản phẩm : </span>
									        <input type="text" value="${price }" name="editprice" placeholder="Nhập giá sản phẩm" required="required"
									               style="width: 100%; padding: 12px; border-radius: 12px; margin-bottom: 10px; box-sizing: border-box;">
									    </div>
									    
									   <!-- Thẻ hiển thị ảnh -->
										<div style="margin-bottom: 10px; display: flex; align-items: center;">
										    <img id="previewImage" style="width: 300px; height: 300px" alt="" src="./uploads/${img}">
										</div>
										
										<!-- Input file để chọn hình ảnh mới -->
										<div style="margin-bottom: 10px; display: flex; align-items: center;">
										    <span style="margin-right: 10px; width: 40%">Chọn hình ảnh : </span>
										    <input type="file" name="editimage" id="imageInput" style="flex-grow: 1; padding: 12px; border-radius: 12px; box-sizing: border-box;" onchange="updatePreview()">
										</div>
										
										<input type="hidden" value="${img }" name="oimg">
...
<script>
    // Hàm được gọi khi có sự thay đổi trong input file
    function updatePreview() {
        // Lấy đối tượng input file
        var input = document.getElementById('imageInput');

        // Lấy đối tượng hiển thị ảnh
        var preview = document.getElementById('previewImage');

        // Kiểm tra xem đã chọn hình ảnh mới chưa
        if (input.files && input.files[0]) {
            // Tạo đối tượng FileReader để đọc file ảnh
            var reader = new FileReader();

            // Đặt sự kiện khi đọc file thành công
            reader.onload = function (e) {
                // Cập nhật nguồn ảnh của thẻ hiển thị
